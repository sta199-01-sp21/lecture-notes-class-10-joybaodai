...
- What is the probability a randomly selected patient received the placebo
  ...
  0% (**disjoint probability**: two events can not occur at the same time)
...
`c()` combines things into *vectors*
...
**Each time you use the `sample()` command, it's a different draw. 
...
** `replace = FALSE` is default, so must specify "TRUE". 
...
Relatively close; as you increase sample size, the value gets closer to the true probability.
